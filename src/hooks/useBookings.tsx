...
export const useBookings = (initialPendingBooking?: PendingBooking | null) => {
  ...
  // Set pending booking if initialPendingBooking changes
  useEffect(() => {
    if (initialPendingBooking) {
      setPendingBooking(initialPendingBooking);
    }
  ...
  const confirmBooking = useCallback(() => {
    ...
    const newBooking: Booking = {
      ...
      image: pendingBooking.image || '/placeholder.svg',
      type: pendingBooking.type || 'package',
      details: {
        confirmationCode: pendingBooking.details?.confirmationCode || 
          `${pendingBooking.destination?.slice(0, 3).toUpperCase()}${Date.now().toString().slice(-4)}`,
        provider: pendingBooking.details?.provider || 'AI Yatri Travel',
        location: pendingBooking.details?.location || pendingBooking.destination,
